...
parser.add_argument('--topic', default="waterlevel", help="Topic to subscribe to, and publish messages to.")
...
def on_message_received(topic, payload):
    if "LOW" in payload.decode('utf-8') or "HIGH" in payload.decode('utf-8'):
        print("Received data from sensor : {}".format(payload))
    global received_count
    received_count += 1
    if "SETTING WATER LEVEL" in payload.decode('utf-8'):
        print("Received instruction from cloud to set water level")
        GPIO.output(18, 0)
        print("Publishing message to user: {}".format('RPi: SETTING VALVE ON'))
        mqtt_connection.publish(
            topic='waterlevel',
            payload='RPi: SETTING VALVE ON',
            qos=mqtt.QoS.AT_LEAST_ONCE)
    elif "HIGH" in payload.decode('utf-8'):
        GPIO.output(18, 1)
        print("Publishing message to user: {}".format('RPi: SETTING VALVE OFF'))
        mqtt_connection.publish(
            topic='waterlevel',
            payload='RPi: SETTING VALVE OFF',
            qos=mqtt.QoS.AT_LEAST_ONCE)
